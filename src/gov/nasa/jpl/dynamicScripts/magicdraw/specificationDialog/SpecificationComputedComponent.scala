...
case class SpecificationComputedComponent[T <: AbstractHierarchicalDisposableTableModel](
  ...
  case class HierarchicaltableComputedComponentFactory() extends HierarchicalTableComponentFactory {
    ...
    def createChildWidgetComponent( _htable: HierarchicalTable, model: DerivedPropertyComputedWidget, row: Int ): Component = {
    ...
    def createChildTreeComponent( _htable: HierarchicalTable, model: DerivedPropertyComputedTreeInfo, row: Int ): Component = {
      ...
      val thread = new Thread() {
        override def run: Unit = {
          ...
          val ( panel, _table ) = SpecificationComputedComponent.createDerivedTreeTablePanel( model.computedDerivedTree, model )
          _table.addMouseListener( DoubleClickMouseListener.createAbstractTreeNodeInfoDoubleClickMouseListener( _table ) )
    ...
    def createChildTreeComponentOrig( _htable: HierarchicalTable, model: DerivedPropertyComputedTreeInfo, row: Int ): Component = {
    ...
    def createChildTableComponent( _htable: HierarchicalTable, model: AbstractDisposableTableModel with AbstractTableModel, row: Int ): Component = {
      ...
      sortableTable.addMouseListener( DoubleClickMouseListener.createAbstractTreeNodeInfoDoubleClickMouseListener( sortableTable ) )
